<template>
    <div id="item" class="main">
        ...
        <div class="box" v-for="(item, idx) in items" :key="idx">
            <tr class="table" :class="{ odd: idx % 2 === 0}" @click="[item.visible = !item.visible, handleModalItem(item), handleModalIndex(idx), handleModalVisible(item.visible)]"> <!--handleModalItem(item) -->
         ...
         <div class="pagination">
            ...
            <div class="pagination-btn" v-for="i in 10" :key="{i}" @click="handlePage(i)">
                <button class="pagination-button" :class="{ currentPage: newPageNum === i}">{{i}}</button><!--v-if="pageNum === i"-->
                <!-- <button class="pagination-button" v-else>{{i}}</button> -->
            </div>
            <button class="pagination-button" @click="handlePageUp()"> &#62;</button>
        </div>
        <Modal v-if="!visible" :item="items" :index="index" :visible="visible" @handleModalVisible="handleModalVisible"/>
    </div>
</template>
